  ...
  return (
    <section className="relative z-10 !pt-0 !pb-0 min-h-screen flex items-center justify-center overflow-hidden bg-[#090909]/40">
      ...
      <div className="relative z-10 section-container text-center">
        ...
        {/* subtitle → buttons: 40px */}
        <motion.p
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.5, ease: [0.22, 1, 0.36, 1] }}
          className="text-white/50 max-w-lg mx-auto mb-10"
          style={{ fontSize: "clamp(1.05rem, 2vw, 1.25rem)", lineHeight: 1.65 }}
        >
        ...
        >
          <button
            ...
            className="px-12 py-5 bg-[#4ade80] text-[#090909] rounded-full text-sm tracking-widest uppercase hover:bg-[#86efac] transition-all duration-300 hover:scale-105 active:scale-95"
          ...
          <button
            ...
            className="px-12 py-5 border border-white/15 text-white/60 rounded-full text-sm tracking-widest uppercase hover:border-[#4ade80]/40 hover:text-white transition-all duration-300"
          >
            Get in Touch
          </button>
        </motion.div>
      ...
      {/* scroll indicator: absolute bottom, stays out of normal flow */}
      <motion.button
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
